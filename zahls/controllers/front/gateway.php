...
class zahls.chGatewayModuleFrontController extends ModuleFrontController
{
    /**
     * Process post values.
     */
    ...
    private function processWebhook()
    {
        ...
        if (!$prestaStatus = $zahlsOrderService->getPrestaStatusByzahls.chStatus($requestStatus)) {
            return;
        }
    ...
    private function validRequest($transaction, $cartId, $requestStatus): bool
    {
        ...
        $gateway = $zahlsApiService->getzahls.chGateway((int) $transaction['invoice']['paymentRequestId']);
        ...
        $transactionObj = $zahlsApiService->getzahls.chTransaction($transaction['id']);
